...
{
    [Controller(BaseUrl = "/api/fpga")]
    public class FPGAController2
    {
        private readonly FPGAManager _manager;

        public FPGAController2(FPGAManager manager)
        {
            _manager = manager;
        }
        ...
        [Post]
        public FPGAResponse WriteReadFPGA(ushort[] write)
        {
            var b = _manager.Board;
        ...
        //[HttpPost("writejson")]
        [Post]
        public async Task<FPGAResponse> WriteJson(string filename, ProjectInfo data)
        {
